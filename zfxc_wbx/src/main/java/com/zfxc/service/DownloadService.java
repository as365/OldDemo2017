...
//							MyDbInfo.getFieldNames()[0], null, null, null, null, "TIME desc, id desc","0,9");
					String sql="select * from "+MyDbInfo.getTableNames()[6]+" order by sname desc, id desc ";
					...
					while (c.moveToNext()) {
						...
						mp3Url=AppConstant.URL.BASE_URL+c.getString(3);
						 String path= Environment.getExternalStorageDirectory().getPath()+"/zfxc_wbx/"+c.getString(3);
						 File f=new File(path);
						 if(!f.exists()){
						HttpDownloader httpDownloader = new HttpDownloader();
					//	Toast.makeText(DownloadService.this, mp3Url, Toast.LENGTH_LONG).show();
						int mp3Result = httpDownloader.downFile(mp3Url, "zfxc_wbx/", c.getString(3));
						 }
